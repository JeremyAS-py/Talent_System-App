...
const aggregatedData = computed(() => {
  ...
  labels.forEach((skill) => {
    ...
    const numericRequerido = skillData.requerido.map((r) => parseFloat(r)).filter((r) => !isNaN(r))
    avgRequerido.push(avg(numericRequerido))

    const numericBrecha = skillData.brecha.map((b) => parseFloat(b)).filter((b) => !isNaN(b))
...
const colaboradorBrechaData = computed(() => {
  ...
  const avgBrechas = labels.map((candidato) => {
    ...
    const numericBrechas = data.brechas.map((b) => parseFloat(b)).filter((b) => !isNaN(b))
